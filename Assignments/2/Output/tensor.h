...
public:
    ...
    template<typename...Dims> tensor(size_t dim, Dims...dims) : width({dim, static_cast<size_t>(dims)...}), stride(sizeof...(dims)+1, 1UL) {
        for (int i=width.size()-1; i!=0; --i) stride[i-1] = stride[i]*width[i];
        data = std::make_shared<std::vector<T>>(stride[0]*width[0]);
        start_ptr = &(data->operator[](0));
    }
